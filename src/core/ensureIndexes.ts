...
export async function ensureIndexes (dbOverride?: Db): Promise<void> {
  ...
  const indexOperations: IndexOperation[] = [
    ...
    { name: 'assets.provider_source_asset_id_unique', run: async () => await assets.createIndex({ provider: 1, source_asset_id: 1 }, { unique: true, name: 'assets_provider_source_asset_id_unique' }) },
    { name: 'assets.group_id', run: async () => await assets.createIndex({ group_id: 1 }) },
    { name: 'assets.group_preferred', run: async () => await assets.createIndex({ group_preferred: 1 }) },
    { name: 'assets.provider', run: async () => await assets.createIndex({ provider: 1 }) },
    { name: 'assets.normalized_repository', run: async () => await assets.createIndex({ normalized_repository: 1 }) },
    { name: 'assets.link_status', run: async () => await assets.createIndex({ link_status: 1 }) },
